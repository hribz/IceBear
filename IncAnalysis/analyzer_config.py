...
class AnalyzerConfig(ABC):
    def __init__(
        ...
        self.max_workers = (
            -1
        )  # The max parallel workers for this analyzer, -1 means no limit.
...
clang_tidy_default_config = {"HeaderFilterRegex": ".*"}
...
class CSAConfig(AnalyzerConfig):
    def __init__(
        self, env: Environment, csa_workspace: Path, config_file: Optional[str] = None
    ):
    ...
    def inc_level_check(self):
        ...
        if self.inc_mode.value >= IncrementalMode.FuncitonLevel.value:
            ...
            result = subprocess.check_output(
                [clang, "-cc1", "-help"], universal_newlines=True, encoding="utf-8"
            )
    ...
    def parse_json_config(self):
        ...
        if self.json_checkers is not None:
            enable_checkers = CSAUtils.get_enable_checkers(
                self.compilers["c"], self.json_checkers
            )
...
class ClangTidyConfig(AnalyzerConfig):
    ...
    def parse_json_config(self):
        if self.json_checkers is not None:
            self.checkers, self.compiler_warnings = (
                ClangTidyUtils.get_checkers_and_warning(
                    self.clang_tidy, self.diagtool, self.json_checkers
                )
            )
...
class CppCheckConfig(AnalyzerConfig):
    ...
    def inc_level_check(self):
        ...
        if self.inc_mode.value >= IncrementalMode.FuncitonLevel.value:
            ...
            if cppcheck is None:
                return
            result = subprocess.check_output(
                [cppcheck, "--help"], universal_newlines=True, encoding="utf-8"
            )
    ...
    def parse_json_config(self):
        if self.json_checkers is not None:
            self.disable_checkers = CppCheckUtils.get_disable_checkers(
                self.cppcheck, self.json_checkers
            )
        if self.json_config:
...
class InferConfig(AnalyzerConfig):
    def __init__(
        self,
        env: Environment,
        infer_workspace: Path,
        config_file: Optional[str] = None,
        checker_file: str = "config/infer_checkers.json",
    ):
...
class GSAConfig(AnalyzerConfig):
    def __init__(
        ...
        gsa_workspace: Path,
        config_file: Optional[str] = None,
        checker_file: str = "config/gsa_checkers.json",
    ):
    ...
    def inc_level_check(self):
        ...
        if self.inc_mode.value >= IncrementalMode.FuncitonLevel.value:
            ...
            result = subprocess.run(
                [
                    gcc,
                    f"--param=analyzer-function-file={param_value}",
                    "-x",
                    "c",
                    "-o/dev/null",
                    main_file_path,
