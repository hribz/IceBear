...
class Repository(ABC):
    ...
    def __init__(
        ...
        self.build_root = (
            build_root if build_root is not None else str(self.src_path / "build")
        )
    ...
    def summary_one_config(self, config: Configuration):
        ...
        analyzers.extend(
            [f"analyze ({inc_level})" for inc_level in self.default_config.inc_levels]
        )
        ...
        config_data.append(
            len(config.diff_file_list)
            if config.incrementable
            else len(config.file_list)
        )
        config_data.extend(
            [
                "%.3lf" % config.prepare_for_inc_info_real_time,
            ]
        )
...
class MultiConfigRepository(Repository):
    def __init__(
        ...
        self.default_config = Configuration(
            ...
            self.env,
            default_options,
            version_stamp="build_0",
            build_path=(
                f"{self.build_root}/{version_stamp}"
                if version_stamp
                else f"{self.build_root}/build_0"
            ),
            ...
            for idx, options in enumerate(options_list):
                self.configurations.append(
                    Configuration(
                        ...
                        options,
                        version_stamp=None,
                        build_path=f"{self.build_root}/build_{idx + 1}",
    ...
    def add_configuration(self, options, build_dir_name=None):
        ...
        self.configurations.append(
            Configuration(
                ...
                self.env,
                options,
                version_stamp=f"build_{idx}",
                build_path=(
                    f"{self.build_root}/{build_dir_name}"
                    if build_dir_name
                    else f"{self.build_root}/build_{idx}"
                ),
                baseline=self.default_config,
                build_type=self.default_build_type,
            )
    ...
    def diff_every_config(self):
        self.process_every_config(
            Configuration.diff_with_other, other=self.default_config
        )
    ...
    def file_status_to_csv(self):
        for config in self.configurations:
            ...
            add_to_csv(
                headers,
                data,
                str(config.workspace / f"file_status_{self.env.timestamp}.csv"),
            )
...
class UpdateConfigRepository(Repository):
    def __init__(
        ...
        self.default_config = Configuration(
            ...
            default_options,
            build_path=(
                self.build_root if out_of_tree else self.src_path
            ),  # Only build in one dir.
    ...
    def process_one_config(self, summary_path=None):
        if not self.default_config.process_this_config(
            self.can_skip_configure, self.has_init
        ):
    ...
    def summary_csv_path(self, specific=False, summary_path=None):
        ret = str(
            self.default_config.workspace
            / f"{os.path.basename(self.name)}_{self.env.analyze_opts.inc}_{self.default_config.version_stamp}"
        )
        if summary_path:
            ret = str(
                self.default_config.workspace
    ...
    def summary_to_csv_specific(self, summary_path):
        ...
        add_to_csv(
            headers,
            data,
            self.summary_csv_path(specific=True, summary_path=summary_path),
            not self.has_init,
        )
    ...
    def summary_to_csv(self, summary_path=None):
        ...
        add_to_csv(
            headers,
            data,
            self.summary_csv_path(specific=False, summary_path=summary_path),
            not self.has_init,
        )
    ...
    def file_status_to_csv(self):
        ...
        add_to_csv(
            ...
            str(
                ...
                / f"file_status_{self.default_config.version_stamp}.csv"
            ),
        )
        ...
        with open(
            str(
                config.preprocess_path
                / f"file_kind_{self.default_config.version_stamp}.json"
            ),
            "w",
        ) as f:
