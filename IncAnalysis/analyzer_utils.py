...
class CSAUtils:
    ...
    def get_enable_checkers(clang, json_checkers):
        ...
        return [checker[0] for checker in list(filter(lambda x: x[1] == CheckerState.ENABLED ,checkers_and_state))]
...
class ClangTidyUtils:
    ...
    @staticmethod
    def get_checker_list(checkers_and_state, checker_description) -> Tuple[List[str], List[str]]:
        compiler_warnings = []
        enabled_checkers = []
        ...
        checkers += ClangTidyUtils._add_asterisk_for_group(
            enabled_checkers,
            set(x[0] for x in checker_description))
...
class CppCheckUtils:
    ...
    def get_disable_checkers(cppcheck, json_checkers):
        ...
        checkers_and_state = determine_checkers_state(checkers, json_checkers)
        return [checker[0] for checker in list(filter(lambda x: x[1] == CheckerState.DISABLED, checkers_and_state))]
