...
class IncrementalMode(Enum):
    ...
    ALL = (
        auto()
    )  # Experiment option, executing [noinc, file, func] incremental strategies.
...
class Environment:
    ...
    def prepare_compiler_path(self, compiler_path):
        ...
        if "cc" in compiler or "c++" in compiler:
            compiler = (
                subprocess.run(["readlink", "-f", compiler_path], capture_output=True)
                .stdout.decode("utf-8")
                .strip()
            )
        if "clang" in compiler:
            self.system_dir[compiler_path] = (
                subprocess.run(
                    [compiler_path, "-print-resource-dir"], capture_output=True
                )
                .stdout.decode("utf-8")
                .strip()
            )
            ...
            gcc_info = (
                subprocess.run(
                    [compiler_path, "-print-search-dirs"], capture_output=True
                )
                .stdout.decode("utf-8")
                .strip()
            )
    ...
    def prepare_env_path(self, ice_bear_path):
        ...
        self.CLANG_PLUS_PLUS = os.path.join(
            clang_bin, os.path.basename(self.CLANG).replace("clang", "clang++")
        )
        ...
        if self.GCC is None or not os.path.exists(self.GCC):
            ...
            self.GXX = os.path.join(
                gcc_bin, os.path.basename(self.GCC).replace("gcc", "g++")
            )
        ...
        if self.DIFF_PATH:
            ...
            self.DIFF_COMMAND.extend(
                [
                    "--old-group-format='%de,%dn %dE,%dN\n'",
...
class ArgumentParser:
    def __init__(self):
        self.parser = argparse.ArgumentParser(
            prog="IceBear", formatter_class=argparse.RawTextHelpFormatter
        )
        ...
        self.parser.add_argument(
            ...
            action="store_true",
            dest="udp",
            help="Use files in diff path to `diff`.",
        )
        ...
        self.parser.add_argument(
            ...
            default=None,
            help="Run analysis only with the analyzers specified. Currently supported analyzers "
